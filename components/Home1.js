                ...
                <View style={tw`px-4 pt-8 h-full`}>
                    ...
                    <View style={tw`flex flex-row justify-between items-start`}>
                        <View style={tw`flex-row`}>
                            ...
                            <View style={tw`ml-4`}>
                                ...
                                <Text style={styles.appBarText2}>{`${profile.isDoctor ? "Dr. " : ""
                                    } ${profile?.name}`}</Text>
                            </View>
                        </View>
                        {/* Hamburger Icon */}
                    ...
                    <View style={[tw`my-5 p-2 bg-white`, styles.search]}>
                        ...
                        <TextInput
                            ...
                            placeholder={
                                profile.isDoctor
                                    ? "Search Patient ..."
                                    : "Search Doctor speciality, hospital or name..."
                            }
                            value={searchedText}
                    ...
                    <ScrollView showsVerticalScrollIndicator={false}>
                        <View style={tw`flex-row items-center justify-between mt-3 mb-5`}>
                            ...
                            <TouchableOpacity
                                onPress={() =>
                                    navigation.navigate("MapScreen", { name: "Maps" })
                                }
                            >
                                <ServiceItem
                                    text={"Nearby Hosptal"}
                        ...
                        {/* Doctors*/}
                        <SectionTitle
                            title={profile.isDoctor ? "Patients" : "Find your Doctor"}
                            pushPage={profile.isDoctor ? "Patients" : "Doctors"}
                            profile={profile}
                        />
                        ...
                        {loadingUsers ? (
                            ...
                            <View>
                                {profile.isDoctor ? (
                                    <View>
                                        {users_filter(filteredData, profile.isDoctor).map(
                                            (userInfo) => (
...
const SectionTitle = ({ title, pushPage = "Home1", profile = null }) => {
    ...
    return (
        <View style={[tw`flex flex-row justify-between items-center px-2`]}>
            ...
            {/* Text2 */}
            <TouchableOpacity
                onPress={() => navigation.navigate(pushPage, { profile })}
            >
...
const PatientItem = ({ userInfo }) => {
    ...
    return (
        <View style={tw`flex-row justify-between items-center my-2`}>
            <Image source={{ uri: userInfo?.image }} style={styles.docImage} />
            <Text style={styles.docText1}>{userInfo?.name}</Text>
            {/* Icon */}
            <TouchableOpacity
                onPress={() => navigation.navigate("PatientProfile", { userInfo })}
            >
...
const DoctorItem = ({ userInfo }) => {
    ...
    return (
        <View style={tw`flex-row justify-between items-center my-4`}>
            ...
            {/* Icon */}
            <TouchableOpacity
                onPress={() => navigation.navigate("DoctorProfile", { userInfo })}
            >
